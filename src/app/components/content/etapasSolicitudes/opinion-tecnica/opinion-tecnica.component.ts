...
import { Bienes } from './propuesta-bien-datos';
...
@Component({
  ...
  // standalone: true,
  selector: 'sg-opinion-tecnica',
  templateUrl: './opinion-tecnica.component.html',
  styleUrls: ['./opinion-tecnica.component.scss'],
})
export class OpinionTecnicaComponent {
  ...
  selectTipoFirma(id: number) {    
    this.documentos.find(el => {
      if (el.id === id) {
        el.firmaElectronica = !el.firmaElectronica;
        console.log(el);
      }      
    })
  ...
  showDropdownMenuAcciones() {
    this.isDropdownMenuAcciones = this.isDropdownMenuAcciones ? false : true;
  }

  hideDropdownMenuAcciones() {
    this.isDropdownMenuAcciones = false;
  }
}
